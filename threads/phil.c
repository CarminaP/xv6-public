...
void *philLive(void *threadid){
  ...
  while(1){
    ...
    sem_wait(&chairs);
    sem_wait(&forks[tid]);
    sem_wait(&forks[(tid + 1) % NUM_PHIL]); //modulo para regresar a 0 si es el ultimo
 ...
 {
    pthread_t philosophers[NUM_PHIL]; //para identificar al hilo
    int rc;
    long t;
    srand(time(NULL)); //time desde ahora a 1970
